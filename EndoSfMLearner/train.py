...
def validate_without_gt(args, val_loader, disp_net, pose_net, epoch, logger):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(val_loader):
        ...
        if epoch == 0:
            ...
            wandb.log({'val Input': wandb.Image(tensor2array(tgt_img[0]).transpose(1, 2, 0))},step=0)
        ...
        """output_writers[i].add_image('val Depth Output',
                                    ...
                                    epoch)"""
        wandb.log({'val Depth Output': wandb.Image(tensor2array(tgt_depth[0][0], max_value=10).transpose(1, 2, 0),step=epoch)},step=epoch)                                
